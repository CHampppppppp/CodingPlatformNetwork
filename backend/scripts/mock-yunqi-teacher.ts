...
async function cleanupExtraTeachers(schoolId: string, scenarioId: string) {
  ...
  const keepNames = MOCK_TEACHERS.map((t) => t.name);
  const toDelete = allTeachers.filter((t) => !keepNames.includes(t.displayName));

  for (const teacher of toDelete) {
    await prisma.teacherProfile.deleteMany({
      where: { nodeId: teacher.id },
    });
    await prisma.graphNode.delete({
      where: { id: teacher.id },
    });
    console.log(`删除教师: ${teacher.displayName}`);
  }
...
async function main() {
  ...
  const deletedCount = await cleanupExtraTeachers(school.id, scenario.id);
  if (deletedCount > 0) {
    console.log(`清理了 ${deletedCount} 名多余教师`);
  }
  ...
  console.log(`云栖小学现有教师总数: ${allTeachers.length}`);
