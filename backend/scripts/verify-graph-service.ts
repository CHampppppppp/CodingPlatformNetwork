...
async function main() {
  ...
  const students = nodes.filter((n: any) => n.nodeType === 'Student');
  const teachers = nodes.filter((n: any) => n.nodeType === 'Teacher');
  const knowledges = nodes.filter((n: any) => n.nodeType === 'Knowledge');
  ...
  const filtered = interactions.filter((i) => {
    const isTS =
      (i.sourceNode.nodeType === 'Teacher' && i.targetNode.nodeType === 'Student') ||
      (i.sourceNode.nodeType === 'Student' && i.targetNode.nodeType === 'Teacher');
    return !(isTS && i.actionType && TEACHER_STUDENT_ACTIONS.includes(i.actionType));
  });

  const studentKnowledgeLinks = filtered.filter(
    (i) => i.sourceNode.nodeType === 'Student' && i.targetNode.nodeType === 'Knowledge'
  );
  const teacherStudentLinks = filtered.filter(
    (i) => i.sourceNode.nodeType === 'Teacher' && i.targetNode.nodeType === 'Student'
  );
