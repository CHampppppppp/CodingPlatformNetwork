...
async function importData() {
  ...
  // Pre-create all schools
  console.log('Pre-creating schools...');
  ...
  // Pre-create all grades
  console.log('Pre-creating grades...');
  const gradeKeys = new Set();
  for (const record of records) {
    if (record[6] && record[8]) {
      const schoolId = schoolMap.get(record[6]);
      gradeKeys.add(`${schoolId}::${record[8]}`);
    }
  }
  
  const gradeMap = new Map();
  for (const gradeKey of gradeKeys) {
    const [schoolId, gradeName] = gradeKey.split('::');
  ...
  console.log('Pre-creating classes...');
  const classKeys = new Set();
  for (const record of records) {
    if (record[6] && record[8] && record[9]) {
      const schoolId = schoolMap.get(record[6]);
      const gradeKey = `${schoolId}::${record[8]}`;
      const gradeId = gradeMap.get(gradeKey);
      classKeys.add(`${gradeId}::${record[9]}`);
    }
  }
  
  const classMap = new Map();
  for (const classKey of classKeys) {
    const [gradeId, className] = classKey.split('::');
