...
async function main() {
  ...
  const insertedCount = await mockResourceRatings(school.id, scenario.id);

  console.log('\n=== 完成 ===');
  console.log(`星洲小学资源评分数据生成完成，共插入 ${insertedCount} 条评分`);
