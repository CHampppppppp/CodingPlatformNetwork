...
```mermaid
erDiagram
    LearningScenario ||--o{ GraphNode : "1:N"
    LearningScenario ||--o{ InteractionSession : "1:N"
    
    GraphNode ||--o| StudentProfile : "1:1"
    GraphNode ||--o| TeacherProfile : "1:1"
    GraphNode ||--o| KnowledgeProfile : "1:1"
    GraphNode ||--o{ StudentCognitiveProfile : "1:N"
    GraphNode ||--o{ Interaction : "作为 source"
    GraphNode ||--o{ Interaction : "作为 target"
    
    InteractionSession ||--o{ Interaction : "1:N"
    InteractionSession }o--|| LearningScenario : "N:1"
    
    School ||--o{ Grade : "1:N"
    Grade ||--o{ SchoolClass : "1:N"
    
    StudentCognitiveProfile ||--o{ StudentCognitiveDimensionScore : "1:N"
    StudentCognitiveDimensionScore }o--|| CognitiveDimensionDef : "N:1"
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           学习场景 LearningScenario                          │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │  code: SHOW_CASE, ONLINE_COURSE, TEACHER_QA, HOME_LEARNING, ...       ││
│  │  nameZh: "展示场景", "学科课程在线学习", "课后线上教师授课答疑"...       ││
│  └─────────────────────────────────────────────────────────────────────────┘│
└────────────┬────────────────────────────────────┬───────────────────────────┘
             │ 1:N                                │ 1:N
             ▼                                    ▼
┌──────────────────────────┐           ┌──────────────────────────┐
│      GraphNode           │           │  InteractionSession      │
│  ┌──────────────────┐   │           │  ┌──────────────────┐   │
│  │ id (CUID)        │   │           │  │ id (CUID)        │   │
│  │ nodeType         │───┼──┐        │  │ scenarioId       │◄──┘
...
└────────────────────────┬────────────────────────────────────────────┘
         ...
         ▼               ▼               ▼
┌─────────────────┐ ┌─────────────┐ ┌──────────────────┐
│ StudentProfile  │ │TeacherProfile│ │ KnowledgeProfile │
│ 学生档案         │ │  教师档案    │ │   知识点档案     │
├─────────────────┤ ├─────────────┤ ├──────────────────┤
│ nodeId (PK, FK) │ │ nodeId (PK) │ │ nodeId (PK)      │
│ learningStyle   │ │ subject     │ │ content          │
│ personality     │ │ teachingGrade│ │ knowledgeType   │
│ groupBehavior   │ │ teachingClass│ │ category        │
└─────────────────┘ └─────────────┘ │ parentNodeId?   │
                                    └──────────────────┘
```
...
```
┌────────────────────────────────────────────────────────────────────┐
│              CognitiveDimensionDef (认知维度定义)                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │ dimensionCode: knowledgeReserve, learningEngagement, ...     │ │
│  │ dimensionNameZh: "知识储备", "学习投入", "认知负荷"...         │ │
...
└─────────────────────────┼───────────────────────────────────────────┘
                          ...
                          ▼
┌────────────────────────────────────────────────────────────────────┐
│        StudentCognitiveDimensionScore (维度得分)                     │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │ profileId (FK) ────────────────────────────┐                 │ │
│  │ dimensionCode (FK) ────────────────────────┼─────────────────┼─┘
│  │ scoreValue: 3.5                             │                 │
...
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │ id (CUID)                                                     │ │
│  │ studentNodeId (FK) ────────► GraphNode (nodeType = Student)  │ │
│  │ profileVersion: "v1.0"                                        │ │
...
└────────────────────────┬───────────────────────────────────────────┘
         ...
         ▼               ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│   Student    │ │   Student    │ │   Teacher    │ │  Knowledge   │
│   zkz801yjc  │ │   zkz801sxt  │ │   zhu...     │ │ Word文档编辑 │
├──────────────┤ ├──────────────┤ ├──────────────┤ ├──────────────┤
│id: S001      │ │id: S002      │ │id: T001      │ │id: K001      │
│type: Student │ │type: Student │ │type: Teacher │ │type: Knowledge│
│scenario: ────┼─┤scenario: ────┼─┤scenario: ────┼─┤scenario: ────┤
│school: zkz   │ │school: zkz   │ │school: zkz   │ │school: null  │
│grade: 8年级  │ │grade: 8年级  │ │grade: 8年级  │ │grade: null   │
│class: 1班    │ │class: 1班    │ │class: 1班    │ │class: null   │
└──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘
       │                │                │                │
       └────────────────┴────────────────┴────────────────┘
                           │
...
| 主表 | 关系 | 从表 | 说明 |
|------|------|------|------|
| **LearningScenario** | 1:N | GraphNode | 场景包含多个节点 |
| **LearningScenario** | 1:N | InteractionSession | 场景包含多个会话 |
| **School** | 1:N | Grade | 学校包含多个年级 |
| **Grade** | 1:N | SchoolClass | 年级包含多个班级 |
| **GraphNode** | 1:1 | StudentProfile | 学生节点的扩展信息 |
| **GraphNode** | 1:1 | TeacherProfile | 教师节点的扩展信息 |
| **GraphNode** | 1:1 | KnowledgeProfile | 知识节点的扩展信息 |
| **GraphNode** | 1:N | Interaction (source) | 作为交互源节点 |
| **GraphNode** | 1:N | Interaction (target) | 作为交互目标节点 |
| **GraphNode** | 1:N | StudentCognitiveProfile | 学生的认知画像历史 |
| **InteractionSession** | 1:N | Interaction | 会话包含多个交互 |
| **StudentCognitiveProfile** | 1:N | StudentCognitiveDimensionScore | 画像包含各维度得分 |
| **CognitiveDimensionDef** | 1:N | StudentCognitiveDimensionScore | 维度定义关联得分 |
