...
* 字段名使用蛇形命名法（snake_case）：如`learning_engagement`

### 7.2 API路径命名规范

* 使用蛇形命名法（snake_case）：单词之间使用下划线分隔
...
* 查询参数使用蛇形命名法（snake_case）：如`teacher_id`

* 路径参数使用蛇形命名法（snake_case）：如`/api/v1/students/:id`
...
* 类名和接口名使用帕斯卡命名法（PascalCase）：如`StudentService`

* 方法名和变量名使用驼峰命名法（camelCase）：如`findAllStudents`

* 常量使用大写字母和下划线（SNAKE_CASE）：如`MAX_PAGE_SIZE`

### 7.6 文件命名规范

* 使用连字符命名法（kebab-case）：全部小写，单词之间使用连字符分隔

* 如`student-controller.ts`

### 7.7 用户面向命名规范

* 所有面向用户、展示的命名使用连字符命名法（kebab-case）
...
1. **Prisma ORM**：利用其类型安全、自动生成查询API和声明式迁移等优势
2. **完整的API接口**：包括学生、教师、知识点、交互、教师-班级关联、数据迁移和AI分析等模块
3. **专门的图谱数据接口**：提供完整的节点和链接数据，满足前端力导向图可视化需求
4. **数据验证**：使用Zod进行请求数据验证，确保数据质量
5. **安全设计**：包括CORS配置、请求验证和日志记录
6. **性能优化**：缓存策略、Prisma性能优化、数据库优化和异步处理
7. **部署策略**：环境变量配置、容器化和CI/CD流水线

本设计考虑了性能、安全性和可扩展性，确保系统能够高效、安全地运行，为交互网络可视化系统提供可靠的后端支持。通过严格遵循RESTful API命名规范，提高了API的可读性和可维护性，便于前后端团队协作和后续开发。
