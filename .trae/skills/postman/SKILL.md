---
name: postman
description: API lifecycle management through Postman. Discover collections, run tests, monitor APIs, validate schemas, and publish documentation across the complete API development workflow. Invoke when working with Postman resources or API testing needs.
---
...
**When you first use this skill, the system will automatically:**
...
**Common first operations that work immediately:**
...
This skill works across multiple environments with proper proxy configuration.
...
This skill provides capabilities to interact with the Postman API to manage the complete API lifecycle. It enables discovery of workspace resources, execution of test collections, monitoring analysis, and more.
...
This skill should be invoked when you:

- Work with Postman, collections, API specifications, or API testing
- Need to create or manage API specifications (OpenAPI, AsyncAPI, Swagger)
- Require uploading or importing API specs to Postman
- Want to generate collections from API specifications
- Need to generate specifications from existing collections
- Want to validate API schemas or compare versions
- Need to create, update, or delete collections and environments
- Require duplicating or organizing collections and environments
- Want to check authentication or security configuration
- Need to create mock servers for prototyping
- Want to run tests or check test results
- Need to see what APIs/collections/specs are available
- Require creating, managing, or analyzing monitors
...
**By default, the skill bypasses all proxy servers** to avoid "403 Forbidden" proxy errors that commonly occur in restricted networks.
...
1. **Run setup validation first**: `python scripts/validate_setup.py`
...
2. If validation fails:
   ...
   - Get your key from https://web.postman.co/settings/me/api-keys
3. For collection management:
   - Run `python scripts/list_collections.py` to see available collections
   - If no collections found:
     - List workspaces: `python scripts/list_workspaces.py`
     - Create new collection
     - Switch workspace in `.env` file
4. Execute specific workflows based on your needs
